...
// Keys namespaced per token so different users on the same browser don't share state
function contactedKey(token)  { return `coned_contacted_${token}`; }
function dismissedKey(token)   { return `coned_dismissed_${token}`; }
function lastReviewKey(token)  { return `coned_last_review_${token}`; }
...
export function useWorkflow(token) {
  ...
  // Load per-user sets when token changes (login/logout)
  useEffect(() => {
    if (!token) {
    ...
    setLastReview(loadLastReview(token));
  }, [token]);

  // Stamp last review time on login
  useEffect(() => {
    if (!token) return;
